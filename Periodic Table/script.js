...
const elements = [
  ...
  { number: 52, symbol: 'Te', name: 'Tellurium', mass: 127.60, group: 16, period: 5 },
  { number: 53, symbol: 'I', name: 'Iodine', mass: 126.90, group: 17, period: 5 },
  ...
  { number: 66, symbol: 'Dy', name: 'Dysprosium', mass: 162.50, group: 3, period: 6 },
  ...
  { number: 109, symbol: 'Mt', name: 'Meitnerium', mass: 278, group: 9, period: 7 },
  { number: 110, symbol: 'Ds', name: 'Darmstadtium', mass: 281, group: 10, period: 7 },
  { number: 111, symbol: 'Rg', name: 'Roentgenium', mass: 282, group: 11, period: 7 },
  { number: 112, symbol: 'Cn', name: 'Copernicium', mass: 285, group: 12, period: 7 },
  { number: 113, symbol: 'Nh', name: 'Nihonium', mass: 286, group: 13, period: 7 },
  { number: 114, symbol: 'Fl', name: 'Flerovium', mass: 289, group: 14, period: 7 },
  { number: 115, symbol: 'Mc', name: 'Moscovium', mass: 290, group: 15, period: 7 },
  ...
  { number: 118, symbol: 'Og', name: 'Oganesson', mass: 294, group: 18, period: 7 }
];

// Get reference to the periodic table container
const periodicTable = document.getElementById('periodicTable');

/**
 * Determines the element category based on its group, atomic number, or symbol.
 */
function getCategory(element) {
  ...
  // Group 1: alkali metals (Hydrogen is typically considered a nonmetal)
  if (element.group === 1) {
    return element.symbol === 'H' ? 'nonmetal' : 'alkali-metal';
  }
  ...
  if (element.group >= 3 && element.group <= 12) return 'transition-metal';
  // Groups 13-16: further classify by known exceptions
  if (element.group >= 13 && element.group <= 16) {
    if (element.group === 13) {
      // Boron is a metalloid; others are post-transition metals
      return element.symbol === 'B' ? 'metalloid' : 'post-transition-metal';
    }
    if (element.group === 14) {
      if (element.symbol === 'C') return 'nonmetal';
      if (element.symbol === 'Si' || element.symbol === 'Ge') return 'metalloid';
      return 'post-transition-metal';
    }
    if (element.group === 15) {
      if (element.symbol === 'N' || element.symbol === 'P') return 'nonmetal';
      if (element.symbol === 'As' || element.symbol === 'Sb') return 'metalloid';
      return 'post-transition-metal';
    }
    if (element.group === 16) {
      if (element.symbol === 'O' || element.symbol === 'S' || element.symbol === 'Se') return 'nonmetal';
      if (element.symbol === 'Te') return 'metalloid';
      return 'post-transition-metal';
    }
  }
  // Fallback category
...
// Create an element card for each element and position it in the grid
elements.forEach(element => {
  const elementDiv = document.createElement('div');
  elementDiv.classList.add('element');
  // Add a class corresponding to its category (for color coding)
  elementDiv.classList.add(getCategory(element));

  // For lanthanides and actinides, override the default grid placement:
  if (element.number >= 57 && element.number <= 71) {
    // Place lanthanides on row 8; center them by starting at column 3
    elementDiv.style.gridRowStart = 8;
    elementDiv.style.gridColumnStart = (element.number - 57) + 3;
  } else if (element.number >= 89 && element.number <= 103) {
    // Place actinides on row 9; center them by starting at column 3
    elementDiv.style.gridRowStart = 9;
    elementDiv.style.gridColumnStart = (element.number - 89) + 3;
  } else {
    // Regular elements use their period and group values
  ...
  // Open the modal with element details when clicked
